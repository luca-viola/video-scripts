...
while getopts ":a:w:h:s:i:o:r:t:" opt; do
  case $opt in
    a)
      ABR="$OPTARG"
      ;;
...
ffmpeg -y -i "$FILE" -c:v ${codec} -profile:v baseline -b:v "$VBR"k -pass 1 -an -s "$W"x"$H" -r $FRAMERATE -pix_fmt yuv420p -threads $THREADS -f mp4 /dev/null

ffmpeg -y -i "$FILE" -c:v ${codec} -profile:v baseline -b:v "$VBR"k -s "$W"x"$H" -r $FRAMERATE -pass 2 -pix_fmt yuv420p -strict -2 -c:a aac -b:a "$ABR"k -movflags faststart -threads $THREADS "$OUTFILE".mp4
...
if [ "$webm" == "1" ]; then
  ...
  ffmpeg -i "$FILE" -codec:v libvpx -quality good -cpu-used 0 -b:v "$VBR"k -qmin 10 -qmax 42 -maxrate "$VBR"k -bufsize "$BUFSIZE"k -vf scale=$W:$H -r $FRAMERATE -an -pass 1 -threads $THREADS -f webm /dev/null

  ffmpeg -i "$FILE" -codec:v libvpx -quality good -cpu-used 0 -b:v "$VBR"k -qmin 10 -qmax 42 -maxrate "$WEBMAXSIZEPASS2"k -bufsize "$WEBBUFSIZEPASS2"k -threads $THREADS -vf scale=$W:$H -r $FRAMERATE -codec:a libvorbis -b:a "$ABR"k -pass 2 -f webm "$OUTFILE".webm
fi
